...
Las cuatro especificaciones están en `../../stxt-web/es/` (versión canónica; espejo en inglés en `en/`):
...
- **`es/stxt-discovery-ref.stxt`** (*STXT-DISCOVERY-SPEC*) — cómo se resuelven schemas y templates: el directorio `.stxt` (§3), la cadena de resolución con sus niveles de proyecto, usuario y sistema (§4), precedencia por namespace (§5), `STXT_PATH` (§6), resolución por documento (§7) y errores de resolución (§8). Es la spec que implementa `SchemaLoader.ts` sobre el `DiscoveryResolver` del núcleo.
...
npm test             # compila y pasa mocha (hoy: 410 tests, todos en verde)
...
- **`extension/SchemaLoader.ts`** — la cara del editor de **STXT-DISCOVERY-SPEC** (`../../stxt-web/es/stxt-discovery-ref.stxt`): la lógica de resolución vive en `DiscoveryResolver` (`@stxt-lang/core` ≥0.6.0) y aquí quedan los dos adaptadores (`vscode.workspace.fs` y el entorno del proceso) más el estado. La cadena de cada documento se resuelve desde su directorio: **todos** los `.stxt/` ascendentes (no solo el primero), después `~/.stxt` y `/etc/stxt` (o `STXT_PATH`, que la sustituye entera), con precedencia **por namespace** (el nivel más cercano gana) y duplicados del mismo nivel como error. Los resultados se cachean por directorio (`RESULTS`), cada nivel lleva su `FileSystemWatcher`, y cualquier cambio provoca `clearCache()` + re-resolución + reanálisis. `ensureSchemasForDocument()` resuelve la cadena del documento al abrirlo, antes de analizarlo — sigue cubriendo el `.stxt/` por encima de la raíz del workspace. La validación es **por documento** (`SchemaLoaderExtension` con el Uri del documento, spec sección 7); `getSchema`/`getSchemas` sin documento son la unión de todo lo resuelto, para los consumidores sin contexto (autocompletado). Los tests inyectan un `DiscoveryEnvironment` aislado para no depender del `~/.stxt` real de la máquina.
